...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    const duration = interaction.options.get('duration').value; // 1d, ...
    ...
    if (isNaN(msDuration)) {
      await interaction.editReply("Please provide a valie timeout duration.");
      return;
    }

    if (msDuration < 5000 || msDuration > 2.419e9) {
      await interaction.editReply("Timeout duration cannot be less than 5 seconds or more tha 28 days.");
      return;
    }

    const targetUserRolePosition = targetUser.roles.highest.position; // Highest role of the target user
    const requestUserRolePosition = interaction.member.roles.highest.position; // Highest role of the user who requested the command
